...
-- All mutations are server-side through the terraform-orchestrator Edge Function.
...
-- Approval is bound to a clean plan; rejection remains possible without one.
...
BEGIN
  ...
  IF p_decision = 'approved' THEN
    SELECT * INTO v_plan FROM public.iac_terraform_runs WHERE package_id = p_package_id AND run_type = 'plan' ORDER BY created_at DESC LIMIT 1;
    IF NOT FOUND OR v_plan.status <> 'succeeded' THEN RAISE EXCEPTION 'latest_terraform_plan_must_be_successful'; END IF;
    IF v_plan.has_destroy OR v_plan.has_replace OR coalesce((v_plan.reconciliation->>'matched')::boolean, false) IS NOT TRUE THEN RAISE EXCEPTION 'terraform_plan_failed_governance_checks'; END IF;
  END IF;
