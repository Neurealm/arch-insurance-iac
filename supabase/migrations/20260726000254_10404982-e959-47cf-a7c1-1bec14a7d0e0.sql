-- BP3.6.1 PATCH: schema-qualify pgcrypto digest() and explicit casts.
-- pgcrypto is installed in schema "extensions"; unqualified digest() fails
-- with "function digest(text, unknown) does not exist" under the pinned
-- search_path=public used by BP3.6 SECURITY DEFINER functions.
...
-- Patch Save RPC: replace only the defective digest call (manifest hash).
...
BEGIN
  ...
  -- Persist result snapshot from calculate()
  ...
  INSERT INTO public.commercial_scenario_comparison_results (
    comparison_id, metric_code, metric_group, fiscal_period, period_sequence, unit,
    baseline_scenario_id, compared_scenario_id, baseline_run_id, compared_run_id,
    baseline_value, compared_value, absolute_variance, percentage_variance,
    variance_direction, comparison_rule, direction_reason
  )
  SELECT _comparison_id, c.metric_code, c.metric_group, c.fiscal_period, c.period_sequence, c.unit,
         ...
         c.variance_direction, NULL, c.direction_reason
  ...
  -- Compute content hash after rows persisted
  ...
  PERFORM public.emit_audit_event(
    v_row.tenant_id,
    'commercial.comparison.saved',
    'commercial_scenario_comparison',
    _comparison_id,
    NULL, NULL, NULL,
    jsonb_build_object(
