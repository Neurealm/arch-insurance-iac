-- BP3.7.3A: Governed failed → draft recovery for sensitivity experiments.
-- Server-authoritative. Never bypasses header/child guards outside the
-- established `app.commercial_sensitivity_op = 'server'` marker.

CREATE OR REPLACE FUNCTION public.commercial_sensitivity_reset_to_draft(
  _experiment_id uuid,
...
BEGIN
  ...
  -- Lock the experiment header for the duration of the transaction
  SELECT * INTO v_row
  ...
  -- Only failed → draft is permitted
  ...
  -- Safety: refuse reset when any executed evidence exists
  SELECT count(*) INTO v_runs_count
    FROM public.commercial_sensitivity_runs
  ...
  SELECT count(*) INTO v_results_count
    FROM public.commercial_sensitivity_results
   WHERE experiment_id = _experiment_id;
  IF v_results_count > 0 THEN
    RAISE EXCEPTION 'sensitivity_reset_has_results';
  END IF;

  -- Enter server lifecycle: bypass header + child guard for the reset only
  PERFORM set_config('app.commercial_sensitivity_op', 'server', true);

  DELETE FROM public.commercial_sensitivity_perturbations
   WHERE experiment_id = _experiment_id;
  ...
  -- Canonical audit event; historical `commercial.sensitivity.failed` remains intact.
...
REVOKE ALL ON FUNCTION public.commercial_sensitivity_reset_to_draft(uuid, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.commercial_sensitivity_reset_to_draft(uuid, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.commercial_sensitivity_reset_to_draft(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.commercial_sensitivity_reset_to_draft(uuid, text) TO service_role;

COMMENT ON FUNCTION public.commercial_sensitivity_reset_to_draft(uuid, text) IS
'BP3.7.3A: Governed failed→draft recovery. Restores a failed, unexecuted sensitivity experiment to draft (only when zero sensitivity runs/results exist), clears failed-attempt lifecycle fields and perturbations under the server lifecycle marker, and emits commercial.sensitivity.reset. Preserves configuration and prior failed audit.';