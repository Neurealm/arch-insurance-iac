...
-- BP1.1C read/update services. All SECURITY DEFINER with explicit search_path.

CREATE OR REPLACE FUNCTION public.list_authorized_tenants()
RETURNS TABLE(
  tenant_id uuid, name text, slug text, status text,
  default_currency text, default_timezone text,
  membership_status text, is_platform_admin boolean
)
...
BEGIN
  ...
  RETURN QUERY
    SELECT t.id, t.name, t.slug, t.status::text,
           t.default_currency, t.default_timezone,
           m.status::text,
           pa
...
RETURNS TABLE(
  ...
  status text, roles jsonb, invited_at timestamptz, joined_at timestamptz,
  total_count bigint
)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public
AS $$
...
BEGIN
  ...
  RETURN QUERY
    WITH base AS (
      SELECT m.id AS membership_id, m.user_id, p.email, p.display_name,
             m.status::text AS status, m.invited_at, m.joined_at,
             COALESCE(
               (SELECT jsonb_agg(jsonb_build_object('code', tr.code, 'name', tr.name))
                FROM public.membership_roles mr
                JOIN public.tenant_roles tr ON tr.id = mr.tenant_role_id
                WHERE mr.membership_id = m.id AND tr.status = 'active'), '[]'::jsonb) AS roles
    ...
    SELECT b.membership_id, b.user_id, b.email, b.display_name, b.status,
           b.roles, b.invited_at, b.joined_at, c.c
    FROM base b CROSS JOIN counted c
    ORDER BY b.joined_at DESC NULLS LAST, b.invited_at DESC NULLS LAST
...
BEGIN
  ...
  RETURN QUERY
    WITH base AS (
      SELECT i.id, i.email, i.status::text, i.expires_at, i.invited_by, i.created_at,
             COALESCE(
               (SELECT jsonb_agg(jsonb_build_object('code', tr.code, 'name', tr.name))
                FROM public.tenant_invitation_roles ir
                JOIN public.tenant_roles tr ON tr.id = ir.tenant_role_id
                WHERE ir.invitation_id = i.id), '[]'::jsonb) AS roles
      FROM public.tenant_invitations i
      WHERE i.tenant_id = p_tenant_id
        AND (p_status IS NULL OR i.status::text = p_status)
    ),
    counted AS (SELECT COUNT(*) c FROM base)
    SELECT b.id, b.email, b.status, b.expires_at, b.invited_by, b.created_at, b.roles, c.c
    FROM base b CROSS JOIN counted c
...
RETURNS TABLE(
  ...
  is_system boolean, member_count bigint, permission_codes text[]
)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public
AS $$
DECLARE uid uuid := auth.uid();
BEGIN
  ...
  RETURN QUERY
    SELECT r.id, r.code, r.name, r.description, r.status::text,
           r.is_system,
           (SELECT COUNT(*) FROM public.membership_roles mr WHERE mr.tenant_role_id = r.id),
           COALESCE(ARRAY(
             SELECT p.code FROM public.tenant_role_permissions trp
             JOIN public.permissions p ON p.id = trp.permission_id
             WHERE trp.tenant_role_id = r.id
             ORDER BY p.code
           ), ARRAY[]::text[])
...
RETURNS TABLE(
  event_id uuid, occurred_at timestamptz, actor_id uuid, actor_email text,
  action text, object_type text, object_id text, reason text, source text,
  correlation_id text, before_state jsonb, after_state jsonb, total_count bigint
)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public
AS $$
DECLARE uid uuid := auth.uid();
BEGIN
  ...
  RETURN QUERY
    WITH base AS (
      SELECT a.id, a.occurred_at, a.actor_id, p.email AS actor_email,
             a.action, a.object_type, a.object_id, a.reason, a.source,
             a.correlation_id, a.before_state, a.after_state
...
DECLARE
  ...
  can_roles boolean;
  can_audit boolean;
  active_members bigint := NULL;
  pending_invitations bigint := NULL;
  active_roles bigint := NULL;
  recent jsonb := NULL;
BEGIN
  IF uid IS NULL THEN RAISE EXCEPTION 'authentication required' USING ERRCODE='42501'; END IF;
  ...
  IF can_audit THEN
    SELECT jsonb_agg(row_to_json(x)) INTO recent FROM (
      SELECT a.id, a.action, a.object_type, a.object_id, a.occurred_at, a.actor_id
      FROM public.audit_events a
      WHERE a.tenant_id = p_tenant_id
...
CREATE OR REPLACE FUNCTION public.update_tenant(
  ...
  p_default_currency text, p_default_timezone text
)
...
BEGIN
  ...
  IF normalized_slug !~ '^[a-z0-9](?:[a-z0-9-]{1,62}[a-z0-9])$' THEN
    RAISE EXCEPTION 'slug must be lowercase alphanumeric with hyphens (3-64 chars)';
  END IF;
  ...
  PERFORM public.emit_audit_event(
    ...
    NULL, 'portal', NULL, before, to_jsonb(after_row)
  );
  RETURN after_row;
