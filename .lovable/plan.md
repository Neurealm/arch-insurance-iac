...
- BP3.4 — Engine — Cash Flow, Working Capital, Break-even, Payback & Sustainability — **Built, Pending Independent Validation** (see `docs/commercial/bp3-4-cash-formulas.md`, page `/commercial/model/cash`, edge function `commercial-run-scenario` with `run_scope=cash`). Cash engine seeded with `PAY_LAG_DAYS=60`, `Q1_ACT_FUND_TIMING_PCT=1.0`, `Q1_TRAVEL_FRONTLOAD_PCT=1.0` across CONS / BASE / UPSIDE. Emits Y1 quarterly cash flow, WC peak trough / max funding, annual NCF (EBITDA proxy per model contract §7), cash conversion, cumulative cash, break-even (EBITDA & cash), payback (year + months, linear interp), and sustainability status.
- BP3.4.1 — Quarterly Cash Formula Reconciliation — **Complete (parity + idempotency verified)** (see `docs/commercial/bp3-4-test-evidence.md`). Corrected cash runs: CONS `2aaab721…`, BASE `521cf9e1…`, UPSIDE `44eab7db…` — each carries `formula_version=bp3.4.1`, 152 inputs / 55 results. Base FY2027 quarterly cash + WC parity to golden = 0.00 variance. Idempotency verified — second execution returned `0 new · 3 reused`. Revenue and P&L runs unchanged.
- BP3.4.2 — Cash Run Supersession Reference Reconciliation — **Complete**. Data-only patch reconciled the BP3.4.1 supersession direction: defective runs (`e79ba517…`, `d225ea9f…`, `93f33caf…`) now have `supersedes_run_id=NULL` while remaining `status='superseded'`; corrected runs (`2aaab721…`, `521cf9e1…`, `44eab7db…`) now reference their matching defective predecessor. `commercial_model_run_header_guard()` extended with a narrowly scoped supersession-link-only allowlist (no other column, including status, may change; self-references forbidden). Zero changes to inputs, results, lineage, hashes, timestamps, or status. Three audit events emitted. Current-run selection unchanged.
- BP3.4.1.PATCH-VERIFY — **PASS** (independent read-only verification).
- BP3.4.EXECUTE — **BP3.4 Runtime Evidence Complete** (see `docs/commercial/bp3-4-test-evidence.md` §BP3.4.EXECUTE). 3 completed cash runs — CONS `2aaab721…` / BASE `521cf9e1…` / UPSIDE `44eab7db…` — with fingerprint `bp3.4.1`, 152 inputs · 55 results each, golden parity max variance `$0.00` across quarterly cash, annual cash, working capital, break-even, payback, sustainability; upstream Revenue and P&L unchanged; supersession lifecycle valid (no cycles, no self-references); 100% lineage; idempotent rerun `0 new · 3 reused`; security and audit clean. BP3.4 — **Built, Pending Independent Validation**. PM-FIN-2026.1 remains **Draft** (activation deferred to BP3.8). Next: `BP3.4.VALIDATE`.
- BP3.5 — Governed Assumption Editing, Change Sets, Validation & Runtime Readiness — **Built and Runtime Executed, Pending Independent Validation** (see `docs/commercial/bp3-5-governed-assumptions.md`, `docs/commercial/bp3-5-test-evidence.md`, page `/commercial/model/assumptions`). Primary change set `1e3de4be-…` (`COST_ESCALATOR_PCT` Conservative 0.030→0.031) validated 2026-07-25 20:18:12 and applied 20:25:48 by `ryancblackwell@outlook.com`; canonical apply-log (`{cash,pnl}`) + 3 audit events (`created`, `validation_passed`, `applied`); scope-aware staleness = Revenue current, P&L stale, Cash stale; historical Revenue/P&L/Cash runs unchanged. Disposable cancellation change set `443baaca-…` (`COST_ESCALATOR_PCT` Conservative proposed 0.032, unapplied) cancelled 2026-07-25 21:13:05 by same actor; canonical `commercial.assumption.change_set.cancelled` audit; item preserved immutable; no apply-log row; effective Conservative remains 0.031; readiness unchanged; zero automatic runs post-cancel. Consolidated BP3.5.1–BP3.5.7 patches all PASS. PM-FIN-2026.1 remains Draft (activation deferred to BP3.8). Next: `BP3.5.VALIDATE`.
- BP3.6 — Scenario Comparison & Delta Views — **Not Started**.
- BP3.7 — Sensitivity Analysis — **Not Started**.
- BP3.8 — Model Activation, Lineage Enrichment, Release Evidence — **Not Started (activation deferred)**.
