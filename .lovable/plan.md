...
- BP3.4.1 — Quarterly Cash Formula Reconciliation — **Patched, Corrected Execution Pending** (see `docs/commercial/bp3-4-test-evidence.md`). Fixed Y1 quarterly cash-cost / travel front-load defect in `computeCashScope` (`baseCostPerQ = annualCost / 4`, Q1 adds full travel load, Q2–Q4 no incremental travel). Cache invalidated via per-scope formula fingerprint (`cash → bp3.4.1`) folded into `commercial_model_run_start`'s hash; revenue and P&L fingerprints unchanged. Prior defective cash runs (`e79ba517…`, `d225ea9f…`, `93f33caf…`) preserved and marked `superseded`. Awaiting user-triggered corrected execution on `/commercial/model/cash` before BP3.4.EXECUTE / BP3.4.VALIDATE.
