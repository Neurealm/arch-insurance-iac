...
- BP3.4 — Engine — Cash Flow, Working Capital, Break-even, Payback & Sustainability — **Built, Execution Pending** (see `docs/commercial/bp3-4-cash-formulas.md`, page `/commercial/model/cash`, edge function `commercial-run-scenario` with `run_scope=cash`). Cash engine seeded with `PAY_LAG_DAYS=60`, `Q1_ACT_FUND_TIMING_PCT=1.0`, `Q1_TRAVEL_FRONTLOAD_PCT=1.0` across CONS / BASE / UPSIDE. Emits Y1 quarterly cash flow, WC peak trough / max funding, annual NCF (EBITDA proxy per model contract §7), cash conversion, cumulative cash, break-even (EBITDA & cash), payback (year + months, linear interp), and sustainability status. Requires user-triggered runtime execution before BP3.4.EXECUTE / BP3.4.VALIDATE.
