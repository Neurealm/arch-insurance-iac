...
export function AvepSidebar() {
  ...
  return (
    ...
    >
      ...
      <Link
        to="/app"
        title={sidebarCollapsed ? "NeuGAIN Command Center" : undefined}
      ...
      >
        ...
        {!sidebarCollapsed && <span className="truncate">NeuGAIN Command Center</span>}
      </Link>
          ...
          return (
            <div key={group} className="mb-3">
              {!sidebarCollapsed && (
                ...
                >
                  {group === "Plan" ? "Phase 1 - Engineering Definition" : group === "Design" ? "Phase 2 - Build and Verify" : group === "Verify" ? "Phase 3 - Prove Readiness and Authorize Advancement" : group}
                </div>
              )}
              <ul>
                {items.map((item) => {
