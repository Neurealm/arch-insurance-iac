...
const dist = (a: number, b: number, c: number, d: number) => [
  { label: "Code Change", value: a },
  { label: "Configuration Change", value: b },
  { label: "Process Change", value: c },
  { label: "Policy Change", value: d },
];

const teams = (a: number, b: number, c: number) => [
  { label: "Checkout Engineering", value: a },
  { label: "Identity Engineering", value: b },
  { label: "Platform Engineering", value: c },
];

export const qualityDetail: Record<string, QualityDetail> = {
  intent: {
    definition: "Whether the stated intent of the incoming work is expressed clearly enough to be evaluated against enterprise context.",
    failureCauses: ["Intent stated as a solution rather than an outcome", "Multiple competing intents in one submission"],
    affectedSections: ["Intent Statement", "Scope Boundary"],
    recommendedActions: ["Request clarification from the submitting owner", "Split multi-intent work into separate intakes"],
    workTypeDistribution: dist(41, 24, 21, 14),
    teamDistribution: teams(38, 34, 28),
    affectedIntakes: ["INT 7003", "INT 7005"],
    recentChanges: ["Intent restated on INT 7003 after clarification response"],
  },
  current: {
    definition: "Whether the current operating state is described with enough precision to compare against the proposed state.",
    failureCauses: ["Current behavior described informally", "No measured baseline referenced"],
    affectedSections: ["Current State", "Evidence Requirements"],
    recommendedActions: ["Attach measured baseline evidence", "Link prior outcome records"],
    workTypeDistribution: dist(46, 27, 16, 11),
    teamDistribution: teams(35, 33, 32),
    affectedIntakes: ["INT 7001", "INT 7004", "INT 7006"],
    recentChanges: ["Baseline latency evidence added to INT 7001"],
  },
  proposed: {
    definition: "Whether the proposed operating state is described in terms that can be reasoned about by downstream evaluation.",
    failureCauses: ["Proposed state omits threshold values", "Rollback behavior unstated"],
    affectedSections: ["Proposed State", "Risk Signals"],
    recommendedActions: ["Capture explicit threshold values", "Document rollback behavior"],
    workTypeDistribution: dist(48, 25, 15, 12),
    teamDistribution: teams(40, 30, 30),
    affectedIntakes: ["INT 7001", "INT 7002"],
    recentChanges: ["Retry ceiling raised from 3 to 5 on INT 7001"],
  },
  scope: {
    definition: "Whether the boundary of the change is defined precisely enough to determine which teams, systems, and conditions are affected.",
    failureCauses: ["Traffic exposure unstated", "Downstream services omitted from the boundary", "Regional scope left implicit"],
    affectedSections: ["Scope Boundary", "Affected Entities", "Dependency Map"],
    recommendedActions: ["State traffic exposure explicitly", "Resolve unmapped downstream services", "Confirm regional applicability"],
    workTypeDistribution: dist(52, 22, 14, 12),
    teamDistribution: teams(44, 29, 27),
    affectedIntakes: ["INT 7001", "INT 7004", "INT 7005", "INT 7007"],
    recentChanges: ["Traffic exposure increased to 25% on INT 7001, activating approval requirements"],
  },
  entity: {
    definition: "Whether referenced systems, services, teams, and policies resolve to canonical enterprise entities.",
    failureCauses: ["Free text service names", "Deprecated aliases still in use", "Ambiguous multi-match candidates"],
    affectedSections: ["Affected Entities", "Enterprise Context"],
    recommendedActions: ["Map unresolved entities to canonical records", "Create a new entity where none exists"],
    workTypeDistribution: dist(49, 26, 13, 12),
    teamDistribution: teams(31, 30, 39),
    affectedIntakes: ["INT 7004", "INT 7005"],
    recentChanges: ["Two entities mapped to canonical records on INT 7004"],
  },
  dependency: {
    definition: "Whether upstream and downstream dependencies affected by the change have been identified.",
    failureCauses: ["Indirect dependencies not traced", "Shared infrastructure not considered"],
    affectedSections: ["Dependency Map", "Related Work"],
    recommendedActions: ["Refresh enterprise context to pull current dependency graph", "Review related prior decisions"],
    workTypeDistribution: dist(47, 28, 14, 11),
    teamDistribution: teams(33, 32, 35),
    affectedIntakes: ["INT 7001", "INT 7004"],
    recentChanges: ["Fraud Decision Service linked as a downstream dependency on INT 7001"],
  },
  risk: {
    definition: "Whether risk signals implied by the change have been surfaced for downstream evaluation.",
    failureCauses: ["Risk signals inferred but not recorded", "Customer impact unstated"],
    affectedSections: ["Risk Signals", "Enterprise Rule Activation"],
    recommendedActions: ["Record identified risk signals explicitly", "Confirm applicable enterprise rules"],
    workTypeDistribution: dist(44, 25, 18, 13),
    teamDistribution: teams(36, 31, 33),
    affectedIntakes: ["INT 7001", "INT 7006"],
    recentChanges: ["Customer impact risk signal recorded on INT 7006"],
  },
  assumption: {
    definition: "Whether assumptions embedded in the proposal have been made explicit rather than left implied.",
    failureCauses: ["Capacity assumptions unstated", "Assumed downstream idempotency"],
    affectedSections: ["Assumptions", "Open Questions"],
    recommendedActions: ["Convert implied assumptions into open questions", "Request evidence supporting each assumption"],
    workTypeDistribution: dist(43, 27, 17, 13),
    teamDistribution: teams(37, 32, 31),
    affectedIntakes: ["INT 7001", "INT 7003"],
    recentChanges: ["Idempotency assumption converted to an open question on INT 7001"],
  },
  evidence: {
    definition: "Whether the evidence required to evaluate the change has been identified and attached.",
    failureCauses: ["Required evidence not yet supplied", "Evidence provided without provenance"],
    affectedSections: ["Evidence Requirements", "Evidence Gaps"],
    recommendedActions: ["Request evidence from the accountable owner", "Attach provenance for supplied evidence"],
    workTypeDistribution: dist(45, 26, 16, 13),
    teamDistribution: teams(34, 33, 33),
    affectedIntakes: ["INT 7001", "INT 7002", "INT 7007"],
    recentChanges: ["Idempotency test results attached to INT 7001"],
  },
  owner: {
    definition: "Whether an accountable owner has been resolved for the incoming work and its affected entities.",
    failureCauses: ["Submitting team differs from accountable owner", "Owner vacancy in the enterprise directory"],
    affectedSections: ["Ownership", "Routing Readiness"],
    recommendedActions: ["Assign an accountable owner before routing", "Escalate unresolved ownership to governance"],
    workTypeDistribution: dist(42, 26, 18, 14),
    teamDistribution: teams(35, 34, 31),
    affectedIntakes: ["INT 7005"],
    recentChanges: ["Accountable owner assigned on INT 7002"],
  },
};
