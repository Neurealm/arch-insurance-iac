...
import {
  ...
  Filter, ArrowUpDown, Target as TargetIcon, BookOpen, UserCheck, FlaskConical} from "lucide-react";
import {
  ...
  return (
    <header className="flex items-center gap-4 px-6 h-[72px] border-b border-white/[0.06] bg-gradient-to-b from-white/[0.025] to-transparent backdrop-blur-xl">
      ...
      <button onClick={onOpenPalette}
        ...
        <Search className="h-3.5 w-3.5" /> Search options, evidence…
...
/* ============================= options data ============================= */
type Opt = {
  id: string; rank: number; label: string; sub: string; window: string; date: string;
  prod: number; yieldStr: string; rev: string; revNum: number; oee: number;
  risk: string; riskTone: string; tech: string; techTone: string;
...
const accentText = (s: number) => s >= 90 ? "text-emerald-300" : s >= 75 ? "text-sky-300" : s >= 60 ? "text-amber-300" : "text-rose-300";

/* ============================= recommended window ============================= */
function RecommendedWindowCard({ opt, onOpen }: { opt: Opt; onOpen: (id: string) => void }) {
  const confidence = useCountUp(opt.confidence);
  const isTopPick = opt.id === "tonight";
  return (
    <GlassCard className="col-span-12 xl:col-span-8 p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div className="text-[13.5px] font-semibold text-white">
            {isTopPick ? "Recommended Maintenance Window" : "Selected Maintenance Window"}
          </div>
        </div>
        <span className={`text-[10px] font-bold tracking-[0.18em] flex items-center gap-1 ${isTopPick ? "text-emerald-300" : "text-amber-300"}`}>
          <Sparkles className="h-3 w-3" /> {isTopPick ? "AI RECOMMENDATION" : "MANUAL OVERRIDE"}
        </span>
      </div>
      ...
      <div className="grid grid-cols-12 gap-4 items-start">
        <div className="col-span-12 md:col-span-5">
          <motion.div
            ...
            className={`text-[36px] leading-none font-bold bg-gradient-to-r bg-clip-text text-transparent ${
              isTopPick ? "from-emerald-300 to-emerald-400" : "from-sky-300 to-indigo-400"
            }`}>
            {opt.label}
          </motion.div>
          <div className="text-[18px] text-white font-semibold mt-1.5">{opt.window}</div>
          <div className="text-[12px] text-slate-400">{opt.date}</div>
          <div className={`mt-3 inline-flex items-center gap-2 px-2.5 py-1 rounded-md border ${isTopPick ? "bg-emerald-500/10 border-emerald-400/30" : "bg-sky-500/10 border-sky-400/30"}`}>
            <span className={`text-[10.5px] ${isTopPick ? "text-emerald-300" : "text-sky-300"}`}>Confidence:</span>
            <span className={`text-[12px] font-bold ${isTopPick ? "text-emerald-200" : "text-sky-200"} tabular-nums`}>{confidence.toFixed(0)}%</span>
            <svg width="40" height="14" viewBox="0 0 40 14" className={isTopPick ? "text-emerald-300" : "text-sky-300"}>
        ...
        <div className="col-span-12 md:col-span-7">
          <p className="text-[12.5px] text-slate-300 leading-relaxed">
            {isTopPick
              ...
              : `You selected ${opt.label}. AI's top pick remains Maintain Tonight (94%). The summary below recomputes against your selection.`}
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 border-t border-white/[0.05] pt-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2.5">
          <SummaryKpi icon={Users} iconBg="bg-sky-500/15 text-sky-300" label="Production Impact" value={`${opt.prod.toLocaleString()} wafers`} onClick={() => onOpen("kpi-prod")} />
          <SummaryKpi icon={ShieldCheck} iconBg="bg-emerald-500/15 text-emerald-300" label="Yield Impact" value={opt.yieldStr} onClick={() => onOpen("kpi-yield")} />
          <SummaryKpi icon={DollarSign} iconBg="bg-amber-500/15 text-amber-300" label="Revenue Impact" value={opt.rev} onClick={() => onOpen("kpi-revenue")} />
          <SummaryKpi icon={Clock} iconBg="bg-indigo-500/15 text-indigo-300" label="Downtime" value={opt.downtime} onClick={() => onOpen("kpi-downtime")} />
          <SummaryKpi icon={AlertTriangle} iconBg="bg-rose-500/15 text-rose-300" label="Risk if Delayed" value={opt.risk} onClick={() => onOpen("kpi-risk")} />
        </div>
      </div>
    </GlassCard>
  );
  ...
  return (
    <GlassCard className="col-span-12 xl:col-span-4 p-4">
      ...
      <div className="space-y-2.5">
        {REASONS.map((r, i) => (
          <motion.button key={r.title} onClick={() => onOpen("reason-" + i)}
...
/* --- Option comparison (interactive selection + sort) --- */
function OptionComparison({ selectedId, setSelectedId, onOpen, sortBy, setSortBy }: any) {
  const sorted = useMemo(() => {
    const list = [...OPTIONS];
    if (sortBy === "score") list.sort((a, b) => b.score - a.score);
    else if (sortBy === "revenue") list.sort((a, b) => b.revNum - a.revNum);
    else if (sortBy === "downtime") list.sort((a, b) => parseFloat(a.downtime) - parseFloat(b.downtime));
  ...
  return (
    <GlassCard className="col-span-12 xl:col-span-8 p-4">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div className="text-[13.5px] font-semibold text-white">Option Comparison <span className="text-slate-500 text-[11px] font-normal ml-1">(AI evaluated 4 options · click a row to select)</span></div>
        <div className="flex items-center gap-1.5 text-[11px] text-slate-400">
          <ArrowUpDown className="h-3 w-3" /> Sort:
          {(["rank", "score", "revenue", "downtime"] as const).map((k) => (
            <button key={k} onClick={() => setSortBy(k)}
              className={`h-6 px-2 rounded-md text-[10.5px] border transition ${sortBy === k ? "bg-sky-500/15 border-sky-400/30 text-sky-200" : "bg-white/[0.03] border-white/[0.06] text-slate-300 hover:bg-white/[0.06]"}`}>
        ...
        <table className="w-full text-[11.5px]">
          <thead>
            <tr className="text-slate-500 text-[10.5px] uppercase tracking-wider">
              ...
              <th className="text-right font-medium pb-2 pr-3">Tech Load</th>
          ...
          <tbody>
            {sorted.map((o) => {
              ...
              const isTopAI = o.id === "tonight";
              return (
                <motion.tr layout key={o.id}
                  onClick={() => setSelectedId(o.id)}
                  onDoubleClick={() => onOpen("opt-" + o.id)}
                  className={`cursor-pointer transition ${
                    isSelected ? "bg-sky-500/[0.08] ring-1 ring-sky-400/50" : isTopAI ? "bg-emerald-500/[0.04] ring-1 ring-emerald-400/30" : "hover:bg-white/[0.03]"
                  }`}>
                  <td className="py-2.5 pl-1">
                    <div className="flex items-center gap-2.5">
                      <span className={`h-6 w-6 rounded-full grid place-items-center text-[10.5px] font-bold text-white ${
                        o.rank === 1 ? "bg-emerald-500" : o.rank === 2 ? "bg-sky-500" : o.rank === 3 ? "bg-amber-500" : "bg-fuchsia-500"
                      }`}>{o.rank}</span>
                      <div>
                        <div className="text-white font-semibold flex items-center gap-2 flex-wrap">
                          {o.label}
                          {isTopAI && <span className="text-[8.5px] font-bold tracking-wider bg-emerald-500/20 text-emerald-300 px-1.5 py-0.5 rounded ring-1 ring-emerald-400/30">AI PICK</span>}
                          {isSelected && !isTopAI && <span className="text-[8.5px] font-bold tracking-wider bg-sky-500/20 text-sky-300 px-1.5 py-0.5 rounded ring-1 ring-sky-400/30">SELECTED</span>}
                        </div>
                        <div className="text-[10px] text-slate-500">{o.sub}</div>
                      </div>
                    </div>
                  </td>
                  <td className="text-right pr-3 text-slate-200 tabular-nums">{o.prod.toLocaleString()}</td>
                  <td className="text-right pr-3 text-slate-200 tabular-nums">{o.yieldStr}</td>
                  <td className="text-right pr-3 text-rose-300 font-semibold tabular-nums">{o.rev}</td>
                  <td className="text-right pr-3 text-slate-200 tabular-nums">{o.oee}</td>
                  <td className={`text-right pr-3 font-semibold ${o.riskTone}`}>{o.risk}</td>
                  <td className={`text-right pr-3 font-semibold ${o.techTone}`}>{o.tech}</td>
                  <td className="text-right">
                    <div className="flex items-center gap-2 justify-end">
                      <div className="w-20 h-1.5 rounded-full bg-white/10 overflow-hidden">
                        <div className={`h-full ${scoreColor(o.score)}`} style={{ width: `${o.score}%` }} />
                      </div>
                      <span className={`font-bold w-6 text-right tabular-nums ${accentText(o.score)}`}>{o.score}</span>
                    </div>
                  </td>
                </motion.tr>
              );
      ...
      <div className="text-[10.5px] text-slate-500 mt-3 flex items-center justify-between border-t border-white/[0.05] pt-2">
        ...
        <span>Press <kbd className="px-1 py-0.5 rounded bg-white/[0.04] border border-white/[0.06] text-slate-300 text-[10px]">1-4</kbd> to select · double-click for details.</span>
      </div>
    </GlassCard>
  );
}

/* --- Evidence (filtered + tone-filter) --- */
const SIGNALS = [
...
function EvidencePanel({ onOpen, query, setQuery, onlyAlerts, setOnlyAlerts }: any) {
  const filtered = SIGNALS.filter((s) =>
    s.label.toLowerCase().includes((query || "").toLowerCase()) &&
    (!onlyAlerts || s.note !== "Stable")
  );
  return (
    <GlassCard className="col-span-12 xl:col-span-4 p-4">
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className="flex-1 flex items-center gap-1.5 px-2 h-7 rounded-md bg-white/[0.03] border border-white/[0.06]">
          <Search className="h-3 w-3 text-slate-500" />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter signals…"
            className="flex-1 bg-transparent text-[11px] text-slate-200 placeholder:text-slate-500 outline-none" />
        </div>
        <button onClick={() => setOnlyAlerts(!onlyAlerts)} title="Only alerts"
          className={`h-7 px-2 rounded-md border text-[10.5px] flex items-center gap-1 ${onlyAlerts ? "bg-rose-500/15 border-rose-400/30 text-rose-200" : "bg-white/[0.03] border-white/[0.06] text-slate-300 hover:bg-white/[0.06]"}`}>
          <Filter className="h-3 w-3" /> Alerts
        </button>
      </div>
      <div className="divide-y divide-white/[0.04]">
        {filtered.length === 0 && <div className="py-3 text-center text-[11px] text-slate-500">No signals match</div>}
        {filtered.map((s) => (
...
/* --- Rationales --- */
type Rationale = { id: string; icon: any; iconColor: string; title: string; bullets: string[]; impact: string; impactTone: string; };
...
/* --- Delay chart with horizon slider --- */
function DelayImpactChart({ horizon, setHorizon }: any) {
  const data = useMemo(() => Array.from({ length: horizon + 1 }, (_, i) => ({
    d: i,
    ...
    <GlassCard className="p-3.5 h-full">
      <div className="flex items-center justify-between mb-1.5">
        ...
        <div className="flex items-center gap-2 text-[10.5px] text-slate-400">
          Horizon
          <input type="range" min={20} max={60} step={5} value={horizon} onChange={(e) => setHorizon(+e.target.value)}
            className="w-20 accent-sky-400 h-1" />
          <span className="text-slate-200 tabular-nums w-10 text-right">{horizon}d</span>
        </div>
      </div>
      ...
      <div className="h-[140px] relative">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
            ...
            <XAxis dataKey="d" tick={{ fontSize: 9, fill: "#64748b" }} ticks={[0, Math.round(horizon / 2), horizon]} tickFormatter={(v) => `${v}`} axisLine={{ stroke: "#1e293b" }} tickLine={false} />
...
const STEPS = [
  ...
  { n: 2, title: "Schedule Maintenance", sub: "Window confirmed" },
...
const APPROVERS = [
  ...
  { name: "Quality", icon: BadgeCheck },
  { name: "Maintenance", icon: Hammer },
  { name: "Operations", icon: ClipboardCheck },
];

function FooterStrip({ approvals, setApproval, approveAll, currentStep, onSubmit, onExport, canSubmit }: any) {
  return (
    <GlassCard className="p-4">
      <div className="grid grid-cols-12 gap-4 items-start">
            ...
            {STEPS.map((s) => {
              ...
              return (
                <div key={s.n} className="flex items-start gap-2">
                  <span className={`h-6 w-6 grid place-items-center rounded-full text-[11px] font-bold ring-1 ${
                    done ? "bg-emerald-500/20 ring-emerald-400/40 text-emerald-300"
                      : active ? "bg-sky-500/15 ring-sky-400/30 text-sky-300"
                      : "bg-white/[0.04] ring-white/[0.08] text-slate-500"
                  }`}>
                    {done ? <CheckCircle2 className="h-3.5 w-3.5" /> : s.n}
                  </span>
                  <div>
                    <div className={`text-[11.5px] font-semibold leading-tight ${active ? "text-white" : done ? "text-slate-300" : "text-slate-400"}`}>{s.title}</div>
        ...
        <div className="col-span-12 lg:col-span-4">
          <div className="flex items-center justify-between mb-2.5">
            <div className="text-[12.5px] font-semibold text-white">Required Approvals</div>
            <button onClick={approveAll} className="text-[10.5px] text-sky-300 hover:text-sky-200 flex items-center gap-1">
              <ThumbsUp className="h-3 w-3" /> Approve all
            </button>
          </div>
          <div className="flex items-start gap-3 flex-wrap">
            {APPROVERS.map((a) => {
              const st: ApprovalState = approvals[a.name] || "Pending";
              const tone = st === "Approved" ? "text-emerald-300" : st === "Rejected" ? "text-rose-300" : "text-amber-300";
              return (
                <div key={a.name} className="flex flex-col items-center gap-1 min-w-[68px]">
                  <a.icon className="h-4 w-4 text-slate-400" />
                  <div className="text-[10.5px] text-slate-300">{a.name}</div>
                  <div className={`text-[10px] font-semibold ${tone} flex items-center gap-0.5`}>
                    {st === "Approved" && <CheckCircle2 className="h-3 w-3" />}
                    {st === "Rejected" && <X className="h-3 w-3" />}
                    {st}
                  </div>
                  <div className="flex items-center gap-1 mt-0.5">
                    <button onClick={() => setApproval(a.name, "Approved")} title="Approve"
                      className={`h-5 w-5 grid place-items-center rounded ${st === "Approved" ? "bg-emerald-500/20 text-emerald-300" : "text-slate-400 hover:bg-white/[0.05] hover:text-emerald-300"}`}>
                      <ThumbsUp className="h-3 w-3" />
                    </button>
                    <button onClick={() => setApproval(a.name, "Rejected")} title="Reject"
                      className={`h-5 w-5 grid place-items-center rounded ${st === "Rejected" ? "bg-rose-500/20 text-rose-300" : "text-slate-400 hover:bg-white/[0.05] hover:text-rose-300"}`}>
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col gap-2">
          <button onClick={onSubmit} disabled={!canSubmit}
            className={`h-10 rounded-lg text-[12.5px] font-semibold transition flex items-center justify-center gap-2 ${
              canSubmit ? "bg-sky-500 hover:bg-sky-400 text-white shadow-[0_0_24px_-8px_rgba(56,189,248,0.8)]" : "bg-white/[0.04] border border-white/[0.06] text-slate-500 cursor-not-allowed"
            }`}>
            <Send className="h-4 w-4" /> {canSubmit ? "Submit for Approval" : "Awaiting Approvals"}
          </button>
          <button onClick={onExport}
            className="h-10 rounded-lg bg-white/[0.04] border border-white/[0.08] text-slate-200 text-[12px] font-semibold hover:bg-white/[0.07] transition flex items-center justify-center gap-2">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ y: -12, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -12, opacity: 0 }}
            ...
            <div className="flex items-center gap-2 px-3 h-11 border-b border-white/[0.06]">
              ...
              <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Jump to option or run an action…"
...
export default function AIMaintenanceDecisionCenter() {
  ...
  const [selectedId, setSelectedId] = useState<string>("tonight");
  const [sortBy, setSortBy] = useState<"rank" | "score" | "revenue" | "downtime">("rank");
  const [horizon, setHorizon] = useState(40);
  const [evidenceQuery, setEvidenceQuery] = useState("");
  const [onlyAlerts, setOnlyAlerts] = useState(false);
  const [approvals, setApprovals] = useState<Record<string, ApprovalState>>({ Manufacturing: "Approved" });
  ...
  const setApproval = useCallback((name: string, st: ApprovalState) => {
    setApprovals((prev) => ({ ...prev, [name]: prev[name] === st ? "Pending" : st }));
    flash(`${name} · ${st}`);
  }, []);
  const approveAll = () => {
    const all: Record<string, ApprovalState> = {};
    APPROVERS.forEach((a) => { all[a.name] = "Approved"; });
    setApprovals(all);
    flash("All approvers · Approved");
  };
  const resetApprovals = () => { setApprovals({}); flash("Approvals reset"); };

  const approvedCount = APPROVERS.filter((a) => approvals[a.name] === "Approved").length;
  const rejectedCount = APPROVERS.filter((a) => approvals[a.name] === "Rejected").length;
  const canSubmit = approvedCount === APPROVERS.length && !submitted;
  const currentStep = submitted ? 3 : approvedCount === APPROVERS.length ? 2 : 1;

  const submit = () => {
    if (!canSubmit) { flash("All approvers required"); return; }
    setSubmitted(true);
    flash("Decision submitted · scheduled");
  };

  const exportCSV = useCallback(() => {
    const header = ["Section", "Key", "Value"];
    const rows: any[] = [
      ["Selected", "Option", selectedOpt.label],
      ["Selected", "Window", selectedOpt.window],
      ["Selected", "Date", selectedOpt.date],
      ["Selected", "Confidence", selectedOpt.confidence + "%"],
      ["Selected", "Score", selectedOpt.score],
      ["KPI", "Production", selectedOpt.prod],
      ["KPI", "Yield", selectedOpt.yieldStr],
      ["KPI", "Revenue", selectedOpt.rev],
      ["KPI", "Downtime", selectedOpt.downtime],
      ...OPTIONS.map((o) => ["Option", o.label, `score ${o.score} · conf ${o.confidence}% · rev ${o.rev}`]),
      ...SIGNALS.map((s) => ["Signal", s.label, `${s.value} · ${s.note}`]),
      ...APPROVERS.map((a) => ["Approval", a.name, approvals[a.name] || "Pending"]),
    ];
    const csv = [header, ...rows].map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = "ai-decision-brief.csv"; a.click();
    URL.revokeObjectURL(url);
    flash("Brief exported");
  }, [selectedOpt, approvals]);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (["1","2","3","4"].includes(e.key)) { setSelectedId(OPTIONS[+e.key - 1].id); flash(`Selected · ${OPTIONS[+e.key - 1].label}`); return; }
      if (e.key.toLowerCase() === "a") { approveAll(); return; }
      if (e.key.toLowerCase() === "s") { submit(); return; }
      if (e.key.toLowerCase() === "e") { exportCSV(); return; }
      if (e.key.toLowerCase() === "r") { resetApprovals(); return; }
      if (e.key.toLowerCase() === "t") { setSelectedId("tonight"); flash("Snapped to AI recommendation"); return; }
    };
  ...
  const paletteActions = [
    ...OPTIONS.map((o, i) => ({ label: `Select · ${o.label}`, icon: Star, hint: String(i + 1), run: () => { setSelectedId(o.id); flash(`Selected · ${o.label}`); } })),
    { label: "Snap to AI recommendation", icon: Sparkles, hint: "T", run: () => { setSelectedId("tonight"); flash("Snapped to AI recommendation"); } },
    { label: "Approve all stakeholders", icon: ThumbsUp, hint: "A", run: approveAll },
    { label: "Reset approvals", icon: RotateCcw, hint: "R", run: resetApprovals },
    { label: "Submit for approval", icon: Send, hint: "S", run: submit },
    { label: "Export decision brief (CSV)", icon: Download, hint: "E", run: exportCSV },
    { label: "Open recommendation drawer", icon: Cpu, run: () => openDrawer("opt-" + selectedId) },
    ...(["rank", "score", "revenue", "downtime"] as const).map((k) => ({ label: `Sort by · ${k}`, icon: ArrowUpDown, run: () => setSortBy(k) })),
  ];
        ...
        <div className="flex">
          ...
          <main className="flex-1 min-w-0 px-5 py-4 space-y-3.5">
            ...
            <div className="sticky top-2 z-30">
              <GlassCard className="px-3 py-2 flex items-center gap-2 flex-wrap">
                <div className="flex items-center gap-1.5 text-[11px]">
                  ...
                  <span className={`font-semibold ${selectedId === "tonight" ? "text-emerald-300" : "text-sky-300"}`}>{selectedOpt.label}</span>
                  <span className="text-slate-500 tabular-nums">· score {selectedOpt.score}</span>
                  {selectedId !== "tonight" && (
                    <button onClick={() => { setSelectedId("tonight"); flash("Snapped to AI recommendation"); }}
                      className="ml-1 text-[10.5px] px-1.5 py-0.5 rounded border border-emerald-400/30 text-emerald-300 hover:bg-emerald-500/10">
                      Snap to AI
                    </button>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 text-[11px] text-slate-300">
                  <span className="text-emerald-300 tabular-nums">{approvedCount}</span>
                  <span className="text-slate-500">/ {APPROVERS.length} approved</span>
                  {rejectedCount > 0 && <span className="text-rose-300 tabular-nums">· {rejectedCount} rejected</span>}
                  <div className="w-24 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
                    <motion.div animate={{ width: `${(approvedCount / APPROVERS.length) * 100}%` }}
                      className="h-full bg-gradient-to-r from-emerald-400 to-sky-400" />
                  </div>
                </div>
                <div className="flex-1" />
                <button onClick={approveAll}
                  className="h-8 px-3 rounded-md bg-emerald-500/15 border border-emerald-400/30 text-emerald-200 text-[11.5px] font-medium flex items-center gap-1.5 hover:bg-emerald-500/25">
                  <ThumbsUp className="h-3.5 w-3.5" /> Approve all
                </button>
                <button onClick={resetApprovals}
                ...
                <button onClick={submit} disabled={!canSubmit}
                  className={`h-8 px-3 rounded-md text-[11.5px] font-medium flex items-center gap-1.5 ${
                    canSubmit ? "bg-sky-500/15 border border-sky-400/30 text-sky-200 hover:bg-sky-500/25" : "bg-white/[0.03] border border-white/[0.06] text-slate-500 cursor-not-allowed"
                  }`}>
                  <Send className="h-3.5 w-3.5" /> {submitted ? "Submitted" : "Submit"}
                </button>
            ...
            <div className="grid grid-cols-12 gap-3.5">
              <RecommendedWindowCard opt={selectedOpt} onOpen={openDrawer} />
              <WhyThisWindow onOpen={openDrawer} />
            </div>

            <div className="grid grid-cols-12 gap-3.5">
              <OptionComparison selectedId={selectedId} setSelectedId={setSelectedId} onOpen={openDrawer} sortBy={sortBy} setSortBy={setSortBy} />
              <EvidencePanel onOpen={openDrawer} query={evidenceQuery} setQuery={setEvidenceQuery} onlyAlerts={onlyAlerts} setOnlyAlerts={setOnlyAlerts} />
            </div>

            <div className="grid grid-cols-12 gap-3.5">
              <div className="col-span-12 lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-3.5">
                {RATIONALES.map((r) => <RationaleCard key={r.id} r={r} onOpen={openDrawer} />)}
              </div>
              <div className="col-span-12 lg:col-span-4"><DelayImpactChart horizon={horizon} setHorizon={setHorizon} /></div>
            </div>

            <FooterStrip approvals={approvals} setApproval={setApproval} approveAll={approveAll}
              currentStep={currentStep} onSubmit={submit} onExport={exportCSV} canSubmit={canSubmit} />
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}
              ...
              <ul className="space-y-1 text-[11px] text-slate-300">
                {[
                  ...
                  ["1-4", "Select option"],
                  ["T", "Snap to AI recommendation"],
                  ["A", "Approve all"],
                  ["R", "Reset approvals"],
                  ["S", "Submit for approval"],
                  ["E", "Export decision brief"],
                  ["?", "Toggle this panel"],
                  ["Esc", "Close overlays"],
                ].map(([k, v]) => (
