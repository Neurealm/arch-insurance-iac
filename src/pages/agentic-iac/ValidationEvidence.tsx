...
function VmValidationDetail({ packageId }: { packageId: string }) {
  ...
  const runValidation = async () => {
    ...
    try { const outcome = await persistValidation(selected.id); setRun(outcome.run); setResults(outcome.results); setEvidence(await getEvidenceItems(outcome.run.id)); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to persist validation evidence."); }
    finally { setRunning(false); }
  };

  const close = async () => {
    if (!run || run.status !== "verified" || run.authority !== "server-v1") return;
    setRunning(true); setError(null);
    try { const outcome = await closeValidationRun(run.packageId); setRun(outcome.run); setResults(outcome.results); setEvidence(await getEvidenceItems(outcome.run.id)); if (outcome.run.status !== "closed") setError("Fresh verification did not pass. The change remains open."); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to close the change."); }
    finally { setRunning(false); }
  };
  ...
  const actual = observed ? `${String(observed.powerState)} · provisioning ${String(observed.provisioningState)}` : "No server-verified Azure observation recorded";
