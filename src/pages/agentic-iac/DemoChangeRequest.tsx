...
export default function DemoChangeRequest() {
  ...
  return <main className="mx-auto max-w-[1180px] space-y-5 px-4 py-6 md:px-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]"><div className="space-y-5">
      ...
      <Panel title={revisingTicket ? "4. Additional notes for the agent" : "4. Additional notes"}>{outstanding.length > 0 && <div className="mb-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900"><b>The agent asked for:</b><ul className="mt-1 list-disc space-y-1 pl-5">{outstanding.map((question) => <li key={question}>{question}</li>)}</ul></div>}<textarea value={additionalNotes} onChange={(event) => setAdditionalNotes(event.target.value)} rows={4} placeholder="Answer the agent's questions or add any extra context. This is appended to the ticket text with a timestamp." className="w-full rounded-md border border-slate-300 p-3 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" /><p className="mt-2 text-[11px] text-slate-500">Optional. Your notes are added to the ticket and re-analyzed by the agent. Nothing is changed in Azure.</p></Panel>
