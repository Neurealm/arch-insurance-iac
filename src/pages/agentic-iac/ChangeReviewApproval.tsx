...
function VmPackageReview({ packageId }: { packageId: string }) {
  ...
  const decide = async (decision: ChangeReviewDecision) => { if (!pkg || !eligibleToReview) return; if (comment.trim().length < 10) { setError("A review rationale of at least 10 characters is required for every decision."); return; } setSaving(true); setError(null); try { await reviewVmChangePackage(pkg.id, decision, comment); setComment(""); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to record the review decision."); } finally { setSaving(false); } };
