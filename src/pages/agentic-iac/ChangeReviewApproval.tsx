...
type ReviewReadiness = { tone: "ready" | "attention"; label: string; detail: string };

function reviewReadiness(pkg: VmChangePackage, vm: AzureVirtualMachine | null): ReviewReadiness {
  if (pkg.actionType === "create_vm") {
    if (vm) return { tone: "attention", label: "Name already in use", detail: `Azure already reports a machine at ${pkg.targetName}. Review the saved plan and this current observation before approving.` };
  ...
  if (!vm) return { tone: "attention", label: "Target not currently discovered", detail: "The package target is not visible in the latest Azure discovery. Review the clean saved plan and captured evidence before approving." };
  if (pkg.actionType === "start_vm" && /running/i.test(vm.powerState)) return { tone: "attention", label: "No start currently required", detail: "Azure already reports this VM as running. Confirm the request remains necessary before approving." };
  if (pkg.actionType === "start_vm" && !/(deallocated|stopped)/i.test(vm.powerState)) return { tone: "attention", label: "Confirm current state", detail: `Azure reports ${vm.powerState}. Confirm the VM is safe to start before approval.` };
  return { tone: "ready", label: "Ready for human review", detail: `Azure reports ${vm.powerState}. A clean saved plan and independent approval determine whether this ${pkg.actionLabel.toLowerCase()} package can enter manual execution.` };
}
...
function VmApprovalQueue() {
  ...
  return <div className="min-w-0 p-4">
    <div className="mb-3 flex flex-wrap items-center gap-2"><nav className="text-[12px] text-slate-500"><Link to="/servicenow-intake" className="hover:text-[#1B4F91]">ServiceNow Intake</Link><span className="mx-1.5">/</span><span className="font-medium text-slate-800">Change Review &amp; Approval</span></nav><button type="button" onClick={() => void load()} className="ml-auto inline-flex h-8 items-center gap-1.5 rounded-md border border-[#E2E8F0] bg-white px-2.5 text-[12px] font-medium text-slate-700 hover:bg-slate-50"><RefreshCw className="h-3.5 w-3.5" />Refresh queue</button></div>
...
function VmPackageReview({ packageId }: { packageId: string }) {
  ...
  // Current Azure discovery informs the reviewer, but it is not an action-type
  // gate. The durable backend approval function remains authoritative: it
  // requires a clean, exact, saved HCP plan -- but only to approve.
  // review_iac_terraform_plan has no such requirement for rejecting or
  // requesting changes, so a blocked or missing plan must not hide those two
  // decisions, only disable Approve specifically.
  const eligibilityBlock = !savedPlan ? "An HCP Terraform saved plan has not been created yet." : savedPlan.status !== "succeeded" ? savedPlan.errorMessage || `The HCP Terraform plan is ${savedPlan.status}.` : null;
  const eligibleToReview = !!pkg && pkg.status === "submitted" && hasPlatformAdminRole && !!user && pkg.createdBy !== user.id;
  const canApprove = eligibleToReview && !eligibilityBlock;
  const reviewBlockedReason = !pkg ? "The package is unavailable." : pkg.status === "draft" ? "This draft must be submitted from Change Engineering before it can be reviewed." : pkg.status !== "submitted" ? "This package has already reached a final review state." : !hasPlatformAdminRole ? "Only a platform administrator can record this pilot approval." : pkg.createdBy === user?.id ? "You submitted this package, so self-approval is prohibited." : null;
  const decide = async (decision: ChangeReviewDecision) => { if (!pkg || !eligibleToReview || (decision === "approved" && !canApprove)) return; if (comment.trim().length < 10) { setError("A review rationale of at least 10 characters is required for every decision."); return; } setSaving(true); setError(null); try { await reviewVmChangePackage(pkg.id, decision, comment, savedPlan); setComment(""); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to record the review decision."); } finally { setSaving(false); } };
  ...
  return <div className="min-w-0 p-4">
    ...
    {pkg.status === "submitted" && readiness && <div className={cn("mb-3 rounded-md border px-3 py-2 text-[12px]", readiness.tone === "ready" ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-amber-200 bg-amber-50 text-amber-900")}><strong>{readiness.label}.</strong> {readiness.detail}</div>}
