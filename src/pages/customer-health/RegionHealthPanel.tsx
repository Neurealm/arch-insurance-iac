  ...
  return (
    <div className={cn("relative overflow-hidden rounded-lg border border-slate-200 bg-slate-50", height)}>
      ...
      {regions.map((r) => {
        ...
        const svc = statusStyles[r.serviceStatus];
        return (
          <div key={r.id} className="absolute -translate-x-1/2 -translate-y-1/2" style={{ left: `${r.x}%`, top: `${r.y}%` }}>
            <button
              type="button"
              ...
              className="relative flex cursor-pointer flex-col items-center rounded-md px-1 py-1 transition-transform duration-200 hover:scale-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/70"
            >
        ...
        return (
          <li key={r.id} className="relative">
            <button
              ...
              onMouseEnter={() => setHover(r.id)}
              onMouseLeave={() => setHover((h) => (h === r.id ? null : h))}
              className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-left transition-all duration-200 hover:-translate-y-[1px] hover:border-slate-300 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/70"
            >
