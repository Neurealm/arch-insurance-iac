  ...
  return (
    <div className="space-y-4">
      ...
      <Panel title="Regional Map" subtitle="Select a region to see whether anything you run there is affected">
        <div className="relative h-64 overflow-hidden rounded-lg border border-slate-200 bg-white">
          ...
          {regions.map((r) => (
            ...
            >
              <span className={cn("block h-3 w-3 rounded-full ring-4 ring-slate-200", statusStyles[r.status].dot)} />
            </button>
          ))}
        </div>
      </Panel>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {regions.map((r) => (
          ...
          >
            ...
            <p className="mt-1 text-[11.5px] text-slate-500">{r.note}</p>
          </Interactive>
        ))}
      </div>
    </div>
