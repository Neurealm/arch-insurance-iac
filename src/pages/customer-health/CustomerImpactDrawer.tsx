...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border-t border-slate-800 px-5 py-4 first:border-t-0">
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      <button
        ...
        className="absolute inset-0 bg-slate-950/60 backdrop-blur-[1px]" onClick={onClose}
      />
      <aside
        role="dialog" aria-modal="true" aria-label={context.title}
        className="ch-drawer relative flex h-full w-full max-w-[460px] flex-col border-l border-slate-800 bg-slate-950 shadow-2xl"
      >
        <header className="flex items-start justify-between gap-3 border-b border-slate-800 px-5 py-4">
          <div className="min-w-0">
            <h2 className="truncate text-[15px] font-semibold text-slate-50">{context.title}</h2>
            {context.subtitle && <p className="mt-0.5 text-[11.5px] text-slate-400">{context.subtitle}</p>}
          </div>
          <button
            type="button" onClick={onClose} aria-label="Close panel"
            className="grid h-7 w-7 shrink-0 place-items-center rounded-md border border-slate-800 text-slate-400 transition-colors duration-200 hover:border-slate-600 hover:text-slate-100"
          >
            <X className="h-4 w-4" />
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto">
          <Section title="What is happening?">
            <p className="text-[13px] leading-relaxed text-slate-300">{context.whatIsHappening}</p>
          </Section>
          ...
          <Section title="What of mine is affected?">
            <ul className="space-y-2">
              {context.affected.map((a) => (
                <li key={a.label} className="flex items-start gap-2.5 rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-2">
                  <span className={cn("mt-1.5 h-2 w-2 shrink-0 rounded-full", statusStyles[a.status].dot)} aria-hidden />
                  <div className="min-w-0">
                    <div className="text-[12.5px] font-medium text-slate-100">{a.label}</div>
                    <div className="text-[11.5px] text-slate-400">{a.detail}</div>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <Section title="What are we seeing?">
            <ul className="space-y-2">
              {context.signals.map((s) => (
                <li key={s.label} className="rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-2">
                  ...
                  <p className="mt-1 text-[11.5px] leading-snug text-slate-400">{s.interpretation}</p>
                </li>
              ))}
            </ul>
          </Section>
          ...
          <Section title="What is being done?">
            <ul className="space-y-1.5">
              {context.whatIsBeingDone.map((w) => (
                <li key={w} className="flex gap-2 text-[12.5px] leading-snug text-slate-300">
          ...
          <Section title="Do I need to do anything?">
            ...
            >
              ...
              <p className="mt-1 text-slate-300">{context.customerAction}</p>
            </div>
          </Section>
                  ...
                  <div className="flex flex-col items-center">
                    <span
                      className={cn(
                        ...
                        t.pending ? "border border-slate-600 bg-transparent" : "bg-sky-400",
                      )}
                      aria-hidden
                    />
                    {i < context.timeline.length - 1 && <span className="w-px flex-1 bg-slate-800" aria-hidden />}
                  </div>
                  <div className={cn("pb-3", t.pending && "opacity-60")}>
                    <div className="flex flex-wrap items-baseline gap-2">
                      <span className="text-[12.5px] font-medium text-slate-100">{t.stage}</span>
                      <span className="font-mono text-[11px] text-slate-500">{t.at}</span>
                    </div>
                    <p className="text-[11.5px] leading-snug text-slate-400">{t.note}</p>
                  </div>
                </li>
              ))}
            </ol>
          ...
          <div className="border-t border-slate-800 px-5 py-3">
            <button
              ...
              className="flex w-full items-center justify-between rounded-md px-1 py-1 text-left text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 transition-colors duration-200 hover:text-slate-300"
            >
              Technical details
            ...
            {showTechnical && (
              <dl className="mt-2 space-y-1.5">
                {context.technical.map((t) => (
                  <div key={t.label} className="flex justify-between gap-4 rounded-md border border-slate-800 bg-slate-900/60 px-3 py-2">
                    <dt className="text-[11.5px] text-slate-400">{t.label}</dt>
                    <dd className="text-right font-mono text-[11.5px] text-slate-200">{t.value}</dd>
                  </div>
                ))}
              </dl>
            )}
