          ...
          <li key={e.id}>
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-[14px] font-semibold text-slate-100">{e.title}</span>
              </div>
              <div className="mt-1 text-[11px] text-slate-500">Started {e.started} · Updated {e.updated} · Next update {e.nextUpdate}</div>
              <p className="mt-2 max-w-4xl text-[12.5px] leading-relaxed text-slate-300">{e.summary}</p>
              <dl className="mt-3 grid gap-3 border-t border-slate-800 pt-2.5 text-[11px] sm:grid-cols-4">
                ...
                <div><dt className="text-slate-500">Affected deployment</dt><dd className="text-slate-200">{e.affectedDeployment}</dd></div>
                <div><dt className="text-slate-500">Affected dependency</dt><dd className="text-slate-200">{e.affectedDependency}</dd></div>
                <div><dt className="text-slate-500">Provider reference</dt><dd className="font-mono text-slate-200">{e.providerReference}</dd></div>
              </dl>
            </Interactive>
          </li>
        ))}
