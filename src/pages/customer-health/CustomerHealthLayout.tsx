  ...
  return (
    <div className="flex min-h-screen w-full bg-slate-950 text-slate-200">
      <aside
        ...
        className="sticky top-0 hidden h-screen w-[92px] shrink-0 flex-col items-center border-r border-slate-800/80 bg-slate-900/50 py-3 md:flex"
      >
        ...
        <NavLink
          ...
          className="mt-2 flex w-[76px] flex-col items-center gap-1 rounded-lg px-1 py-2 text-[10.5px] text-slate-500 transition-colors duration-200 hover:text-slate-200"
        >
      ...
      <ImpactDrawerProvider>
        {(api) => (
          <main className="flex min-w-0 flex-1 flex-col">
            <header className="sticky top-0 z-30 flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 bg-slate-950/90 px-5 py-3 backdrop-blur">
              <div>
                <div className="text-[10px] uppercase tracking-[0.18em] text-slate-500">
                  {tenant.name} · {tenant.service}
                </div>
                <div className="text-[15px] font-semibold text-slate-50">Customer Health Dashboard</div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[11.5px] text-slate-500">Last updated: {tenant.lastUpdated}</span>
                <button
                  type="button" aria-label="Refresh"
                  className="grid h-8 w-8 place-items-center rounded-md border border-slate-800 text-slate-400 transition-colors duration-200 hover:border-slate-600 hover:text-slate-100"
                >
                  <RefreshCw className="h-3.5 w-3.5" />
                </button>
                <button
                  type="button"
                  className="flex h-8 items-center gap-2 rounded-md border border-slate-800 px-3 text-[12px] text-slate-300 transition-colors duration-200 hover:border-slate-600 hover:text-slate-50"
                >
