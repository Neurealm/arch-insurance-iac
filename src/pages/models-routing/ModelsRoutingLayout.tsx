// Global application shell for the neugain.io LLM + Model Routing
// administration plane: fixed left rail with an expanded Models & Routing
// branch, page header, dark navy purpose banner, tenant routing outcomes and
// routable secondary tabs.
...
const SUB_NAV = [
  { to: "/models-routing/overview", label: "Overview" },
  { to: "/models-routing/models", label: "Model Registry" },
  { to: "/models-routing/providers", label: "Providers & Credentials" },
  { to: "/models-routing/policies", label: "Routing Policies" },
  { to: "/models-routing/guardrails", label: "Safety & Guardrails" },
  { to: "/models-routing/evaluations", label: "Evaluations" },
  { to: "/models-routing/cost", label: "Cost & Performance" },
  { to: "/models-routing/access", label: "Access & Security" },
    ...
    <div className="flex min-h-screen bg-[#F7F9FC] text-slate-900">
      ...
      <aside className={cn("sticky top-0 flex h-screen shrink-0 flex-col border-r border-slate-200 bg-[#0B1B33] text-slate-200 transition-all", collapsed ? "w-[64px]" : "w-[230px]")}>
        ...
        <nav className="flex-1 overflow-y-auto py-2" aria-label="Administration">
          ...
          {/* Expanded Models & Routing branch */}
          <div className={cn("mx-2 my-0.5 flex items-center gap-2 rounded-md bg-white/10 px-2.5 py-2 text-[12.5px] font-medium text-white")}>
            <Cpu className="h-4 w-4 shrink-0 text-sky-300" />
            {!collapsed && <><span className="flex-1">Models &amp; Routing</span><ChevronDown className="h-3.5 w-3.5 text-slate-400" /></>}
          </div>
        ...
        {!collapsed && (
          <div className="mx-3 mb-2 rounded-md border border-white/10 bg-white/5 px-3 py-2.5">
            ...
            <button className="mt-1 text-[11px] text-sky-300 hover:underline" onClick={() => toast.info("Platform status page", { description: "All inference gateways and routing services operational." })}>
              View status page
            </button>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white px-5 py-3">
          <div className="flex flex-wrap items-start gap-3">
            <div className="min-w-0 flex-1">
              <h1 className="text-[26px] font-semibold leading-tight tracking-tight text-slate-900">LLM + Model Routing Administration</h1>
              <p className="mt-0.5 max-w-4xl text-[12.5px] text-slate-500">
                Tenant-level configuration of providers, model catalog, routing policies, safeguards, fallback, cost, performance, and evaluation used by neugain.io digital coworkers.
              </p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Btn variant="primary" onClick={() => setWizard(true)} className="h-8"><Plus className="h-3.5 w-3.5" /> Add Provider / Model</Btn>
            </div>
          </div>
        </header>
          ...
          <div className="flex flex-col gap-4 xl:flex-row">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 max-w-5xl text-[12.5px] leading-relaxed text-slate-200">
                The LLM + Model Routing layer is the governed inference control plane for neugain.io. It selects the right model for the
                right task, enforces security and policy constraints, optimizes inference for quality, reasoning depth, latency, and cost,
                and provides resilient fallback when models or providers are unavailable. Routing is provider-agnostic: no workload depends
                on a single vendor, region, or deployment.
              </p>
            ...
            <div className="xl:w-[340px] xl:shrink-0">
              <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">Tenant Routing Outcomes</div>
        ...
        {/* Secondary tabs */}
        <nav className="sticky top-[86px] z-20 flex gap-0.5 overflow-x-auto border-b border-slate-200 bg-white px-5" aria-label="Model routing sections">
          {SUB_NAV.map((t) => (
      ...
      <AddProviderWizard open={wizard} onClose={() => setWizard(false)}
        onActivated={(n) => { setWizard(false); toast.success("Model successfully registered and available for routing evaluation.", { description: n }); }} />

      <GlobalSearch open={search} onClose={() => setSearch(false)}
        onSelect={(drawer) => { setSearch(false); navigate({ pathname: "/models-routing/overview", search: `?drawer=${drawer}` }); }} />
      ...
      <InspectDrawer open={!!o} onClose={() => setOutcome(null)} objectType="Tenant Routing Outcome" name={o?.label ?? ""} status={o?.result} statusTone="ok"
        tabs={[{ id: "m", label: "Measurement", content: o ? (
          <div>
            ...
            <KV rows={[["Calculation", o.calculation], ["Target", o.target], ["Current result", o.result], ["Trend", o.trend], ["Contributing providers", o.contributors]]} />
