...
describe("CAE.105 rollout placements", () => {
  ...
  it("renders one Hear More control per placement", () => {
    renderRollout();
    expect(screen.getAllByRole("button", { name: /^Hear More\. Play narration$/ })).toHaveLength(PLACEMENTS.length);
  });

  it("plays, pauses, resumes and stops a placement narrative", async () => {
    const user = userEvent.setup();
    renderRollout();
    const [first] = screen.getAllByRole("button", { name: /^Hear More\. Play narration$/ });
  ...
  it("plays only one placement narrative at a time and stops on navigation", async () => {
    const user = userEvent.setup();
    renderRollout();
    const buttons = screen.getAllByRole("button", { name: /^Hear More\. Play narration$/ });
  ...
  it("an unauthorized caller gets no narration for a placement", async () => {
    ...
    const user = userEvent.setup();
    renderRollout();
    await user.click(screen.getAllByRole("button", { name: /^Hear More\. Play narration$/ })[2]);
