...
export function useCommercialPnlBundle(tenantId: string | null) {
  return useQuery({
    enabled: !!tenantId,
    queryKey: commercialQueryKey(tenantId, "pnl-bundle"),
    queryFn: async () => {
      ...
      const revenueRuns = allRuns.filter((r) => r.run_scope === "revenue");
      ...
      const latestRevenueByScenario = new Map<string, ModelRun>();
      for (const r of revenueRuns) {
        if (r.status !== "completed") continue;
        if (!latestRevenueByScenario.has(r.scenario_id)) latestRevenueByScenario.set(r.scenario_id, r);
      }
...
export function useTriggerPnlRun(tenantId: string | null) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: commercialQueryKey(tenantId, "pnl-bundle") });
    },
  });
}
